...
namespace FoodBank.Web.Controllers
{
    ...
    public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
    {
        ...
        if (user != null)
        {
            ...
            {
                ModelState.AddModelError("UserName", "Your Account needs to be confirmed, please check your email.");
            }
        }
    ...
    //[AllowAnonymous]
    //public ActionResult RegisterAccountant()
    //{
    //    var model = new RegisterAccountantModel();
    //    return View(model);
    //}
    ...
    //[AllowAnonymous]
    //[ValidateAntiForgeryToken]
    //public async Task<ActionResult> RegisterAccountant(RegisterAccountantModel model)
    //{
    //    //Check if firm Name exists 
    //    if (_accountantBusiness.DoesFirmExist(model.FirmName))
    //    {
    //        ModelState.AddModelError("FirmName", "The Firm Name already exists");
    //    }
    ...
    //        {
    //            await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
    //            var firmId = await _accountantBusiness.RegisterAccountant(model);
    //            await _accountantBusiness.CreateAccountant(user, firmId);
    ...
    public async Task<ActionResult> GenerateEmailConfirmation(AppUser user)
    {
        ...
        var accountCreationModel = new AccountCreationMailModel()
        {
            ActivationUrl = callbackUrl,
            FirstName = user.FirstName,

        };
        accountCreationModel.ToRecipients.Add(user.Email);

        await _emailService.SendAccountCreation(accountCreationModel);
    ...
    public ActionResult RegenerateEmailConfirmation()
    {
        var model = new RegenerateEmailConfirmation();
        return View(model);
    }

    public async Task<ActionResult> RegenerateEmailConfirmation(RegenerateEmailConfirmation model)
    {
        var user = await UserManager.FindByNameAsync(model.Email);
        if (user != null)
        {
    ...
    public async Task<ActionResult> ConfirmEmail(Guid userId, string code)
    {
        ...
        if (result.Succeeded)
        {
            ...
            if (user.ChangePasswordOnFirstLogin)
            {
                ...
                return RedirectToAction("ResetPassword", "Account", new { userId = user.Id, code = resetCode });
            }
        }
    ...
    public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
    {
        if (ModelState.IsValid)
        {
            ...
            var resetModel = new ResetPasswordMailModel();
            resetModel.FirstName = user.FirstName;
            resetModel.ResetLink = callbackUrl;
            resetModel.ToRecipients.Add(user.Email);

            await _emailService.SendResetPassword(resetModel);
