...
namespace FoodBank.Web.Areas.Supplier.Controllers
{
    public class ListingsController : BaseFoodController
    {
        ...
        public async Task<ActionResult> Index(Guid? branchId)
        {
            ...
            {
                model = await _listingBusiness.GetListingsBySupplier(AuthenticatedUser.CompanyFirmId);
            }
            else
            {
                model = await _listingBusiness.GetListingsBySupplierBranch(branchId.Value);
            }
            return View(model);
        }
        ...
        public ActionResult Create()
        {
            ...
            model.SupplierBranchId = AuthenticatedUser.AuthSupplierModel.SupplierBranchId;
            model.SupplierBranches = _dropDownBusiness.GetBranches(AuthenticatedUser.CompanyFirmId);
            return View(model);
        }
        ...
        public ActionResult Create(ListingCreateModel model)
        {
            ...
            return RedirectToAction("Edit","Listings", new {area="Supplier",id = id});
        }
